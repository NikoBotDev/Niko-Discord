...
class TagUpdateCommand extends Command {
    ...
    super('tag-update', {
      ...
      args: [
        ...
        },
        {
          id: 'content',
          match: 'rest',
          type: validate('string', content => content.length >= 10 && content.length <= 1000),
          prompt: {
            start: 'What\'ll be the new content for this tag?\n',
            retry: 'Sorry, the content must be at least 10 characters long with a maximum of 1000 characters\n',
          },
        },
      ],
    });
  ...
  */
  async exec(msg, {name, content}) {
    const [count] = await this.client.db.tags.update({content}, {
      where: {
        name,
