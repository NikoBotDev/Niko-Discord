...
module.exports = class EvalCommand extends Command {
  ...
  async exec(msg, {script}) {
    ...
    const response = await this.makeResultMessages(this.lastResult, hrDiff, script);
    if (!response) return;
    const color = message.member && message.member.roles.highest.color !== 0 ?
  ...
  async makeResultMessages(result, hrDiff, input) {
    ...
    if (inspected.length >= 1500) {
      ...
      embed.setDescription(
          `:inbox_tray: Input\n\`\`\`javascript\n${input}\n\`\`\`\n:outbox_tray: Output ${hrDiff[0] > 0 ? `${hrDiff[0]}s ` : ''}${hrDiff[1] / 1000000}ms.\n\`\`\`javascript\nOutput is too long and was uploaded to pastebin: ${a ? await a.text() : '[Fail]'}\n\`\`\``
      );
    } else {
      embed.setDescription(
          `:inbox_tray: Input\n\`\`\`javascript\n${input}\n\`\`\`\n:outbox_tray: Output ${hrDiff[0] > 0 ? `${hrDiff[0]}s ` : ''}${hrDiff[1] / 1000000}ms.\n\`\`\`javascript\n${inspected}\n\`\`\``
